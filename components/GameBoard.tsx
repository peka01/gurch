...
const GameBoard: React.FC<GameBoardProps> = ({ players: initialPlayers, onQuit }) => {
  ...
  const handleConfirmSwap = (cards: Card[]) => {
      ...
      if (gameState.gamePhase !== GamePhase.FIRST_SWAP_DECISION && 
          ...
          console.log(`[DEBUG] Safety check failed - wrong game phase: ${gameState.gamePhase}, expected FIRST_SWAP_DECISION, FIRST_SWAP_ACTION, or OTHERS_SWAP_ACTION`);
          (handleConfirmSwap as any)._inProgress = false;
          return;
      }
  ...
  const handleBotPlay = () => {
    ...
    setTimeout(() => {
        ...
        let newRoundWinnerId = gameState.roundWinnerId;
        if (gameState.lastPlayedHand.length === 0 || cardsToPlay[0].value > gameState.lastPlayedHand[0].value) {
  ...
  const validatePlay = (cards: Card[]): boolean => {
    ...
    if (winningPlays.length > 0) {
        console.log("[VALIDATION] Player has a winning play available.");
        const isAWinningPlay = cards[0].value > leadHand[0].value && cards.every(c => c.rank === cards[0].rank);
        if (!isAWinningPlay) console.log("[VALIDATION] FAILED: Player must make a winning play.");
        else console.log("[VALIDATION] PASSED: Player correctly made a winning play.");
        return isAWinningPlay;
    }

    const higherCards = player.hand.filter(c => c.value > leadHand[0].value);
    if (higherCards.length > 0) {
        console.log("[VALIDATION] Player can 'beat and sacrifice'.");
        ...
        const playedHighest = cards.some(c => c.rank === highestBeatingCard.rank && c.suit === highestBeatingCard.suit);
        const playedLowest = lowestCards.every(lc => cards.some(pc => pc.rank === lc.rank && pc.suit === lc.suit));

        if (!playedHighest || !playedLowest) console.log("[VALIDATION] FAILED: Incorrect 'beat and sacrifice' play.");
        else console.log("[VALIDATION] PASSED: Correct 'beat and sacrifice' play.");
        return playedHighest && playedLowest;
    }
  ...
  const findBestPlayForBot = (hand: Card[], leadHand: Card[]): Card[] => {
    ...
    if (winningPlays.length > 0) {
        ...
        return winningPlays[0]; // Play lowest winning hand
    }

    // 2. Check for "beat and sacrifice"
    const higherCards = hand.filter(c => c.value > leadValue);
    if (higherCards.length > 0) {
        const highestBeatingCard = higherCards.sort((a,b) => b.value - a.value)[0];
        const sortedHand = [...hand].filter(c => c !== highestBeatingCard).sort((a,b) => a.value - b.value);
        const lowestCards = sortedHand.slice(0, leadCount - 1);
        return [highestBeatingCard, ...lowestCards];
    }

    // 3. Sacrifice lowest-ranked group of cards
    const possibleSacrifices = Object.values(handByRank)
        .filter(group => group.length >= leadCount)
        .map(group => group.slice(0, leadCount));

    if (possibleSacrifices.length > 0) {
        possibleSacrifices.sort((a, b) => a[0].value - b[0].value);
        return possibleSacrifices[0];
    }
    
    // Fallback if no same-rank group can be sacrificed
